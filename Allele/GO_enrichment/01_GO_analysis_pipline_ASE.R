...
setwd("~/Dropbox/PhD Folder/Virtual Lab Book/MERN_Analysis/GO_Analysis/ASE_ASM")
...
GO_annotations <- read.table("../transcript/GOs_all_transcription_genes_as_background.txt", header = T)
...
my_genes <- read.csv("./Gene_Lists/ASE_repro_gene_list.txt",header=T)
#my_genes <- read.csv("./Gene_Lists/ASE_sterile_gene_list.txt",header=T)
#my_genes <- read.csv("./Gene_Lists/ASE_both_gene_list.txt",header=T)
...
# 10/15(for repro only ASE)
# 5/8 (for sterile only ASE)
# 69/116 (for both ASE)
...
write.table(REVIGO,"./Enriched_GOs/enriched_GOs_ASE_repro_only.txt",row.names = F,sep = "\t",quote = F)
#write.table(REVIGO,"./Enriched_GOs/enriched_GOs_ASE_sterile_only.txt",row.names = F,sep = "\t",quote = F)
#write.table(REVIGO,"./Enriched_GOs/enriched_GOs_ASE_both.txt",row.names = F,sep = "\t",quote = F)
