...
GO_annotations <- read.table("../meth/GOs_all_methylated_genes_as_background.txt", header = T)
...
#my_genes <- read.csv("repro_amr_genes_unique_list.csv",header=F)
#my_genes <- read.csv("nonrepro_amr_genes_unique_list.csv",header=F)
my_genes <- read.csv("both_amr_genes.csv",header=F)
...
# 147/190 (for repro only ASM)
# 80/98 (for sterile only ASM)
# 86/97  (for both ASM)
...
#write.table(REVIGO,"enriched_GOs_ASM_repro_only.txt",row.names = F,sep = "\t",quote = F)
#write.table(REVIGO,"enriched_GOs_ASM_sterile_only.txt",row.names = F,sep = "\t",quote = F)
write.table(REVIGO,"enriched_GOs_ASM_both.txt",row.names = F,sep = "\t",quote = F)
