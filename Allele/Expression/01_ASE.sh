...
for file in $(ls *.bam)
do
  	base=$(basename ${file} ".WASP_Filtered_sorted.bam")
...
#  java -Xms128m -Xmx1024m -Djava.io.tmpdir=$TMPDIR -jar /cm/shared/apps/picard/2.6.0/picard.jar CreateSequenceDictionary R=GCF_000214255.1_Bter_1.0_genomic.fa O=GCF_000214255.1_Bter_1.0_genomic.fa.dict 
...
REF_FILE=/scratch/monoallelic/hm257/allele_specific_MERN/GCF_000214255.1_Bter_1.0_genomic.fa    

for file in $(ls *.bam)
do
  	base=$(basename ${file} "_marked.bam")
    java -jar /cm/shared/apps/gatk/3.8/GenomeAnalysisTK.jar \
        -T SplitNCigarReads \
        -R ${REF_FILE} \
        -I ${file} \
        -o ${base}_split.bam \
